...
public class UploadAgentDeliveryActivity extends AppCompatActivity {
	...
	public void initializer() {
		...
		prefs = PreferenceManager.getDefaultSharedPreferences(UploadAgentDeliveryActivity.this);
	...
	private boolean checkList() {
		...
		try {
			if (this.mSharedPrefs.getBoolean("cloudServices", false)) {
				try {
					if (this.mSharedPrefs.getString("internetAccessModes", null).toString().equals(null)) {
						Toast.makeText(UploadAgentDeliveryActivity.this, "Please Select Prefered Data Access Mode!", Toast.LENGTH_LONG).show();
						return false;
						...
						try {
							if (this.mSharedPrefs.getString("licenseKey", null).equals(null) || this.mSharedPrefs.getString("licenseKey", null).equals(XmlPullParser.NO_NAMESPACE)) {
								...
								Toast.makeText(UploadAgentDeliveryActivity.this, "License key not found!", Toast.LENGTH_LONG).show();
								return false;
							}
							try {
								...
								Toast.makeText(UploadAgentDeliveryActivity.this, "Portal URL not configured!", Toast.LENGTH_LONG).show();
								return false;
							} catch (Exception e) {
								//this.checkListReturnValue = "Portal URL not configured!";
								Toast.makeText(UploadAgentDeliveryActivity.this, "Portal URL not configured!", Toast.LENGTH_LONG).show();
			...
			Toast.makeText(UploadAgentDeliveryActivity.this, "Cloud Services not enabled!", Toast.LENGTH_LONG).show();
			return false;
	...
	private class AsyncCallWS extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... aurl) {
			...
			try {
				...
				Cursor accounts = db.rawQuery("SELECT * FROM " + Database.Agt_FactoryDeliveries + " where " + condition + "", null);
				...
				if (accounts.getCount() > 0) {
					...
					UploadAgentDeliveryActivity.this.soapResponse = new SoapRequest(UploadAgentDeliveryActivity.this).createDelivery(DeliveryInfo);
					error=soapResponse;
					errorNo=prefs.getString("DelerrorNo", "");
					DeliveryNo=soapResponse;
					try{
					if (Integer.valueOf(UploadAgentDeliveryActivity.this.errorNo).intValue()<0) {
				...
				if (accounts1.getCount() > 0) {
					...
					while (accounts1.moveToNext()) {
						...
						try {
							soapResponse = new SoapRequest(UploadAgentDeliveryActivity.this).DeliverBatch(DeliveryNo, BatchNo);
							error=soapResponse;
							errorNo=prefs.getString("DelerrorNo", "");
				...
				soapResponse = new SoapRequest(UploadAgentDeliveryActivity.this).SignoffDelivery(DeliveryNo);
				error=soapResponse;
		...
		protected void onPostExecute(String unused) {
			...
			try {
				...
				if (Integer.valueOf(soapResponse).intValue() > 0) {
					...
					Toast.makeText(UploadAgentDeliveryActivity.this, "Data Uploaded Successfully !!!", Toast.LENGTH_LONG).show();
					new Restart().execute();
					return;
				}
			} catch (NumberFormatException e) {
				...
				if(error.equals("-8080")){
					Toast.makeText(UploadAgentDeliveryActivity.this, "Server Not Available !!", Toast.LENGTH_LONG).show();
					finish();
					return;
				}
				...
				{
				Toast.makeText(UploadAgentDeliveryActivity.this, error, Toast.LENGTH_LONG).show();
				finish();
				}
			}
	...
	public void getdata(){
		...
		try {
			...
			Cursor accounts = db.rawQuery("select * from " + Database.Agt_FactoryDeliveries + " WHERE "
	...
	private class Restart extends AsyncTask<Void, Void, String>
	{
		...
		protected void onPostExecute(String result)
		{
			...
			mIntent = new Intent(getApplicationContext(),UploadAgentDeliveryActivity.class);
