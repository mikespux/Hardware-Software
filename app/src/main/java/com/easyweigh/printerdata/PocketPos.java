package com.easyweigh.printerdata;

import android.util.Log;

import com.easyweigh.printerutils.NumberUtil;
...
//      offset++;
     // buffer[offset] = typeFrame;
      ...
      offset = convertSpecialBytes(srcBuff, srcOffset, srcLen, buffer, 0);

      //offset = convertSpecialBytes(checkSum, 0, checkSum.length, buffer, 0);
...
//    }
  ...
  //  if (fontSet != FONT_DEFAULT)
   ...
   //font = new byte[]{B_ESC, (byte)0x21, fontSet};
    font = new byte[]{fontSet};
   fontalign = new byte[]{align};
  // fontlinespace = new byte[]{linespace};
